import time
import case_numbers as case_numbers
import os
...
class DriverScrapping():
    def __init__(self):
        ...
        path_to_driver = Path("C:/Users/filip/PycharmProjects/arbitration_bot/arbitr_parser/chromedriver.exe").resolve()
        ...
        if len(find_el) == 1:
            ...
            for i in range(len(find_casetype)):
                if find_casetype[i].text == "Определение":
                    amount_of_opred += 1
                    num = i
            if amount_of_opred == 1 and (len(find_casetype) == 2 or len(find_casetype) == 3 or len(find_casetype) == 4):
                s1 = '//*[@id="chrono_list_content"]/div[2]/div/div[' + str(num + 1) + ']/div[2]/h2/a/span[2]'
                s2 = '//*[@id="chrono_list_content"]/div[2]/div/div[' + str(num + 1) + ']/div[2]/h2/a/span'
                if self.checkexistsbypath(s1) == False:
                    s1 = s2
                ...
                for i in range(len(right_name_of_pdf_file)):
                    if name_of_pdf_file[0:22] == right_name_of_pdf_file[i] and \
                            float(price_output_with_point[27:]) >= float(1000000):
    ...
    def all_types(self):
        ...
        date = datetime.now()
        current_datetime = str(date.day-1) + '.' + str(date.month) + '.' + str(date.year)
